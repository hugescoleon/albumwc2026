...
import { Save, Image as ImageIcon, Type, Target, Plus, Trash2, X, Users, Search, Eye, Copy, Check, Megaphone, Heart } from 'lucide-react';
import { albumData } from '../data/albumData';
import { getSectionStickerIds } from '../utils/albumUtils';

// Pre-calculate valid sticker IDs to avoid reconstructing this set on every render
...
export const AdminSettings = ({ config, onUpdate, onClose }) => {
      ...
      if (!error && data) {
        ...
        if (data.length > 0) {
          ...
          if (!countErr && counts) {
            const countsMap = {};
            counts.forEach(s => {
              countsMap[s.user_id] = (countsMap[s.user_id] || 0) + 1;
            });
  ...
  const [visibleStickersCount, setVisibleStickersCount] = useState(30);

  useEffect(() => {
    setVisibleStickersCount(30);
  }, [stickerFilter]);
  ...
  return (
    <div className="fixed inset-0 z-[200] bg-[#0e0e0e]/98 p-4 overflow-y-auto">
      <div className="max-w-2xl mx-auto space-y-8 pb-24">
        <header className="flex items-center justify-between sticky top-0 bg-[#0e0e0e]/90 py-4 z-10">
        ...
        <section className="space-y-4">
          ...
          <GlassCard className="p-6 space-y-4">
             ...
             <div className="max-h-60 overflow-y-auto space-y-1 pr-2">
                {(() => {
                  const filteredStickers = Object.entries(localConfig.stickerNames || {})
                  ...
                  const displayedStickers = filteredStickers.slice(0, visibleStickersCount);

                  return (
                    <>
                      {displayedStickers.map(([id, name]) => (
                      ...
                      {filteredStickers.length > visibleStickersCount && (
                        <button
                          type="button"
                          onClick={() => setVisibleStickersCount(prev => prev + 50)}
                          className="w-full py-2.5 mt-2 bg-white/5 hover:bg-white/10 text-gold hover:text-gold-light border border-dashed border-white/20 rounded-xl text-[10px] font-black uppercase tracking-wider transition-all cursor-pointer"
                        >
                          Cargar más (+50 cromos)
                        </button>
                      )}
                    </>
                  );
              ...
              <p className="text-[9px] text-gray-600 font-bold italic">
                Usa el buscador para filtrar cromos por ID o Nombre.
              </p>
          </GlassCard>
        </section>
                        ...
                        return (
                          <tr key={u.id} className="text-xs hover:bg-white/5 transition-colors">
                            <td className="py-3.5 pr-2">
                              ...
                              <div className="text-[10px] text-gray-500 mt-0.5">📞 {u.phone ? u.phone : 'Sin registrar'}</div>
